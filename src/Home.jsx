...
export default function Home(){
 ...
 const colorArray=[
  ...
  const handleCheckBoxChange = (e)=>{
    ...
    return(
     <>
     <div style={{display:'flex'}}>{checkedInfo.checkedItems.map((item)=>(
     <><h6>{item}</h6>
     <button className='cancelButton'>x</button></>))}
     </div>
     <button style={{width:'200px'}} onClick={setMenu}>select</button>
     <ul className={showMenu?'dropdown-data-show':'dropdown-data-hide'}>
     {colorArray.map((item)=>(
         <li>
             <input type='checkbox' id={item.id} value={item.name} name={item.name} onChange={handleCheckBoxChange}/>
